...
public class JurosSimplesServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Tarefa de Juros Simples</h1>");
            out.println("<p><b>Nome:</b> Adriano Kenji Koyama Augusto</p>");
            out.println("<p><b>RA:</b> 1290481922029</p>");
            double valorP = Double.parseDouble(request.getParameter("vp"));
            double juros = Double.parseDouble(request.getParameter("juros"));
            double tempo = Double.parseDouble(request.getParameter("tempo"));
            
            double vf = valorP * (1 + (juros/100) * tempo);
